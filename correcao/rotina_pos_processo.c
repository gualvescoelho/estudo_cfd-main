...
void correlacao(int desejado_1, int desejado_2, int ponto_1, int ponto_2, int sub, char nome_in[250],int colunas, int linhas, int itmin);
...
void achar_time(float time_escolhido,int colunas,int itmin,char nome_in[250],int linhas);
	...
	do
	{
		...
		system("cls || clear");
			...
			{		
				...
				case 9: //gerar arquivo para gera��o de grafico
...
void achar_time(float time_escolhido,int colunas,int itmin,char nome_in[250],int linhas)
{
	...
	arq_out = fopen("arquivo_saida_coord","w+b");
	arq = fopen(nome_in,"r");
...
void arq_saida(int qtd_coord, int linhas, int itmin, char nome_in[250])
{
...
void exibir_coordenadas()
{
	...
	while(aux != NULL) 
	{
		printf("%d:\t%f\t%f\t%f\n",aux->coluna, aux->x, aux->y, aux->z); //exibir a posi��o das colunas e as respectivas posi��es
		aux = aux->proxima; //pula para o proximo endere�o
	}
}
